...
const editorConfig = {
  ...
  MENU_CONF: {
    uploadImage: {
      fieldName: "file",
      server: "/diyadmin/upload",
      headers: {
        Authorization: localStorage.getItem("diy-admin-token"),
      },
      customInsert(res, insertFn) {
        // 从 res 中找到 url，然后插入图片
        // const url = `https://admin.memtoys.com/diyadmin/image/${res.data}`;
        const url = `https://admin.memtoys.com/diyadmin/download?fileId=${res.data}`;
        insertFn(url);
      },
    },
  },
...
watch(
  ...
  (data) => {
    if (data.datas.id) {
      ...
      getArticleDetailById(data.datas.id)
        .then((res) => {
